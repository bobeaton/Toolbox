...
void CorrespList::Add( const wxChar* pszSrc, const wxChar* pszTar, bool bCount ) // Add a new corresponcence to the end of the list
	{
	if ( bCount )
		{
...
void Guesser::AddCorrespondence( const wxChar* pszSrc, const wxChar* pszTar, int iFreq ) // Make a correspondence to the list
	...
	if ( iGuessLevel == 0 ) // 1.5.8va 
		return; // 1.5.8va 
	corlstKB.Add( pszSrc, pszTar, false );
	}
